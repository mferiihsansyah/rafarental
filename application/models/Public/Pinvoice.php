...
class Pinvoice extends CI_Model {
	...
	{
		$query = $this->db->query("select * from bb_invoice where handphone_inv = '$hp'");
	...
	function load_car($kode)
	{
		$this->db->select('*');
		$this->db->from('bb_invoice');
		$this->db->join('bb_vehicle', 'bb_vehicle.id_vh = bb_invoice.id_vh');
	...
	public function upload_imgbukti($where,$data,$table)
	{
		$this->db->where($where);
