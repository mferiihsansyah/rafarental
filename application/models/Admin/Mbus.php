...
class Mbus extends CI_Model {
	...
	public function load_laporanbus()
	{
		$this->db->select('*,count(*) as total');
		$this->db->from('bb_vehicle');
		$this->db->join('bb_invoice','bb_invoice.id_vh=bb_vehicle.id_vh');
		$this->db->group_by('bb_vehicle.id_vh');
		$this->db->where('bb_invoice.status_inv',2);
		$query = $this->db->get();
		return $query->result();
	}

	public function load_tipe()
	{
		$this->db->select('*');
