...
class Minvoice extends CI_Model {
	...
	*/
	...
	public function load_data_process()
	{
		$this->db->select('*');
		$this->db->from('bb_invoice');
		$this->db->where('status_inv', '8');

		$query = $this->db->get();
		return $query->result();
	}

	public function load_data_cancel()
	{
		$this->db->select('*');
		$this->db->from('bb_invoice');
		$this->db->where('status_inv', '5');

		$query = $this->db->get();
		return $query->result();
	}

	public function load_invoice($kode)
	{
		$this->db->select('*');
		$this->db->from('bb_invoice');
	...
	*/
	...
	public function update_process($where,$data,$table)
	{
		$this->db->where($where);
